...
<h1 class="page-header">
    <a href="/{{ config('admin-notifications.route') }}">@lang('admin-support::admin.list-title')</a>
</h1>
...
<div class="sm:rounded-lg bg-white shadow mb-10">
    <div class="py-5 px-4 border-b border-gray-200">
        ...
        <span class="text-xl leading-6 font-medium text-gray-900">
            @lang('admin-notifications::admin.edit-title')
        </span>
    </div>
</div>

<div class="my-4 mx-2 md:mx-10 pl-10">
	<form action="/{{ config('admin-notifications.route') }}/{{ $notification->id }}" method="POST">
		@csrf
        @method('PATCH')
		<div class="flex">
			<div class="w-full">
				<textarea class="w-full rounded-b rounded-tl px-4 py-4 border-2 focus:outline-none
					...
					name="message" rows="3" id="read">{{ old('message') ?? $notification->message }}</textarea>
			</div>
			<div class="pt-6 ml-1">
